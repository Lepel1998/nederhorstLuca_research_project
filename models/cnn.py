...
This module contains the set-up of the model using Keras API (Kasinathan et al., 2021; Nochnack, 2022).
"""
import os
import time
import numpy as np
...
IMAGE_DATASET_PATH = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'processed_dataset')
image_dataset = tf.keras.utils.image_dataset_from_directory(IMAGE_DATASET_PATH,
                                                            batch_size=32,
                                                            image_size=(227, 227))
print(f'Inladen vanuit folder geluk!: {IMAGE_DATASET_PATH}')
...
LOG_PROCESS = os.path.join(os.path.dirname(__file__), '..',
                           'log_process_CNN')
